...
class CustomEvaluator(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, T = None, lamb = 0, means = 1.0):
        if T is not None:
            losses = 0
            for t in range(T):
                losst = self.evaluator(input[t, ...], target)
                losses += losst
            loss = losses / T
            if lamb != 0:
                MMDLoss = torch.nn.MSELoss()
                y = torch.zeros_like(input).fill_(means)
                Loss_mmd = MMDLoss(input, y) # L_mse
            else:
                Loss_mmd = 0
            loss = (1 - lamb) * loss + lamb * Loss_mmd # L_Total
            self.model_time_step = T
        else:
...
class SwapEvaluator(torch.nn.Module):
    def __init__(self, evaluator: torch.nn.Module, model_type: str, model_time_step: int, dynamic: bool):
        super().__init__()
        self.evaluator = evaluator
        self.model_type = model_type
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, lamb = 0, means = 1.0):
        if self.dynamic:
            losses = 0
            for t in range(self.model_time_step):
                losst = self.evaluator(input[t, ...], target)
                losses += losst
            loss = losses / self.model_time_step
            if lamb != 0:
                MMDLoss = torch.nn.MSELoss()
                y = torch.zeros_like(input).fill_(means)
                Loss_mmd = MMDLoss(input, y) # L_mse
                ...
                Loss_mmd = 0
            loss = (1 - lamb) * loss + lamb * Loss_mmd # L_Total
            self.model_time_step = self.model_time_step
        else:
            loss = self.evaluator(input, target)
        if self.model_type == "bptt":
            return loss
...
class CustomUpdataEvaluator(torch.nn.Module):
    def __init__(self, evaluator: torch.nn.Module, model_type: str, model_time_step: int):
        super().__init__()
        self.evaluator = evaluator
        self.model_type = model_type
    ...
    def forward(self, input: torch.Tensor, old_input: torch.Tensor, target: torch.Tensor, known_classes, T = None, lamb = 0, means = 1.0):
        if T is not None:
            losses = 0
            for t in range(T):
                losst = self.evaluator(input[t, ...], old_input[t], target, known_classes)
                losses += losst
            loss = losses / T
            if lamb != 0:
                MMDLoss = torch.nn.MSELoss()
            ...
            loss = self.evaluator(input, old_input, target, known_classes)
        if self.model_type == "bptt":
            return loss
        elif self.model_type == "rate":
            loss = loss.detach() + (loss / self.model_time_step - (loss / self.model_time_step).detach())
...
class BicUpdataEvaluator(torch.nn.Module):
    def __init__(self, evaluator: torch.nn.Module, model_type: str, model_time_step: int):
        super().__init__()
        self.evaluator = evaluator
        self.model_type = model_type
    ...
    def forward(self, input: torch.Tensor, old_input: torch.Tensor, target: torch.Tensor, known_classes, lamda, T = None, lamb = 0, means = 1.0):
        if T is not None:
            losses = 0
            for t in range(T):
                losst = self.evaluator(input[t, ...], old_input[t], target, known_classes, lamda)
                losses += losst
            loss = losses / T
            if lamb != 0:
                MMDLoss = torch.nn.MSELoss()
            ...
            loss = self.evaluator(input, old_input, target, known_classes, lamda)
        if self.model_type == "bptt":
            return loss
        elif self.model_type == "rate":
            loss = loss.detach() + (loss / self.model_time_step - (loss / self.model_time_step).detach())
